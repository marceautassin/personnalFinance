...
export async function ingestStatement(args: IngestStatementArgs): Promise<IngestionResult> {
  ...
  const reconciliation = reconcile({
    ...
    transactions: extracted.map(t => ({ amountCents: t.amountCents as Cents })),
  })
  ...
  try {
    ...
    await deletePdfByHash(hash).catch(() => { /* best-effort */ })
    throw err
  }
  ...
  for (const oldHash of replacedHashes) {
    await deletePdfByHash(oldHash).catch(() => { /* best-effort */ })
  }
